...
class LikelihoodExpansion:
    ...
    def _inv_cov(self):
        ...
        if not np.allclose(cov, cov.T, rtol=1e-12, atol=1e-12):
                        warnings.warn("[ForecastKit] `cov` is not symmetric; proceeding as-is (no symmetrization).",
                                      RuntimeWarning)
        ...
        try:
            ...
            if cond > 1e12:
                warnings.warn(f"[ForecastKit] `cov` is ill-conditioned (cond≈{cond:.2e}); "
        ...
        try:
            ...
            warnings.warn("[ForecastKit] `cov` inversion failed; using pseudoinverse.",
                          RuntimeWarning)
