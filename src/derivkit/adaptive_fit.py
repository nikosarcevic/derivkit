...
class AdaptiveFitDerivative:
    ...
    :param function: The function to be differentiated. Must return scalar or
        vector output.
    :type function: callable
    :param central_value: The point at which the derivative is evaluated.
    :type central_value: float
    :param diagnostics_data: The diagnostic data collected during the fitting
        process, if requested. Defaults to `None`.
    :type diagnostics_data: dict, optional
    :param min_used_points: The default minimum number of samples required
        for fitting. This is used to ensure that the polynomial fit has enough
        data points to be meaningful.
    :type min_used_points: int
    """
    ...
    def compute(
        ...
        :param derivative_order: The order of the derivative to compute
            (default is 1). Must be 1, 2, 3, or 4. Higher orders are not
            currently supported and will raise a `ValueError`; the computation
            will not proceed.
        :type derivative_order: int, optional
        :param min_samples: Minimum number of total samples to start with.
            Must be large enough to support the fit and any fallback strategies.
            Default is 7.
        :type min_samples: int, optional
        :param fit_tolerance: Maximum acceptable relative residual for the
            polynomial fit. Default is 0.05, i.e 5%.
        :type fit_tolerance: float, optional
        :param include_zero: Whether to include the central point in the
            sampling grid. Default is `True`.
        :type include_zero: bool, optional
        :param fallback_mode: Strategy if the fit fails to meet the tolerance at
            the minimum sample count. Options are:
        ...
        :type fallback_mode: str, optional
        :param floor_accept_multiplier: Tolerance multiplier used in "auto"
            fallback mode. Default is 2.0.
        :type flooc_accept_multiplier: float, optional
        :param diagnostics: Whether to return diagnostic information such as
            residuals, fit values, and pruning status. Default is `False`.
        :type diagnostics: bool, optional
        :param n_workers: Number of worker to use in multiprocessing.
            Default is 1 (no multiprocessing).
        :type n_workers: int, optional

        :return: The estimated derivative(s). If `diagnostics` is `True`,
            returns a tuple with the derivative array and a dictionary
            of diagnostic data.
        :rtype: :class:`np.ndarray`, (:class:`np.ndarray`, dict)
        """
    ...
    def get_adaptive_offsets(
        ...
        :param central_value: The central value around which to generate
            offsets. If None, uses `self.central_value`.
        :type central_value: float, optional
        :param base_rel: Base relative step size as a fraction of the
            central value. Default is 1%.
        :type base_rel: float
        :param base_abs: Base absolute step size for small central values.
            Default is 1e-6.
        :type base_abs: float
        :param factor: Factor by which to increase the step size for each
            offset. Default is 1.5.
        :type factor: float, optional
        :param num_offsets: Number of offsets to generate. Default is 10.
        :type num_offsets: int, optional
        :param max_rel: Maximum relative step size as a fraction of the
            central value. Default is 5%.
        :type max_rel: float, optional
        :param max_abs: Maximum absolute step size. Default is 1e-2.
        :type max_abs: float, optional
        :param step_mode: Mode for determining step sizes: "auto", "relative",
            or "absolute". Defaults to "auto".
        :type step_mode: str, optional
        :param x_small_threshold: Threshold below which the central value is
            considered small. Default is 1e-3.
        :type x_small_threshold: float, optional

        :return: An array with absolute step sizes (positive only),
            not including the central 0 step.
    ...
    def _build_x_offsets(
        ...
        """
        Construct a symmetric array of sampling offsets around `central_value`
        ...
        :param central_value: The central point around which the offsets are
            generated. This is the point at which the derivative will be evaluated.
        :type central_value: float
        :param derivative_order: The order of the derivative to be computed.
            Determines the minimum polynomial degree, which affects the number
            of required sample points.
        :type derivative_order: int
        :param include_zero: Whether to include the central point (zero offset)
            in the sampling grid.
        :type include_zero: bool
        :param min_samples: The minimum number of total sampling points to start
            with before pruning. This interacts with `self.min_used_points`
            (default = 5), which is the minimum number of points required to
            perform a polynomial fit, and with the derivative order to determine
    ...
    def _fit_once(
        ...
        :param x_vals: Sample x values used in the fit.
        :type x_vals: :class:`np.ndarray`
        :param y_vals: function values corresponsing to `x_vals`.
        :type y_vals: :class:`np.ndarray`
        :param derivative_order: Order of the derivative to fit.
        :type derivative_order: int

        :return: Dictionary containing fit result, polynomial object,
    ...
    def _maybe_accept_at_floor(
        ...
        """
        Determine whether to accept a polynomial fit computed at the minimum
        stencil size ("floor").
        ...
        :param last_residuals: Residuals from the last attempted polynomial fit.
            If None, acceptance is based only on fallback_mode and `at_floor`
            status.
        :type last_residuals: :class:`np.ndarray`, optional
        :param at_floor: Whether the algorithm has reached the minimum allowed
            number of sample points.
        :type at_floor: bool
        :param fit_tolerance: The user-specified threshold for acceptablei
            residuals. Typically a small value like 1e-4.
        :param fit_tolerance: float
        :param fallback_mode: str
        ...
        :param floor_accept_multiplier: Tolerance multiplier for
        ...
        :type floor_accept_multiplier: float
    ...
    def _fallback_derivative(self, derivative_order, n_workers=1):
        """
        Compute the derivative using a finite difference method when adaptive
        fitting fails.
        ...
        :param derivative_order:
            The order of the derivative to compute. Must be one of the orders
            supported by :class:`derivkit.finite_difference.FiniteDifferenceDerivative`
            (currently 1–4).
        :type derivative_order: Number of worker to use in multiprocessing.
            Default is 1 (no multiprocessing).
        :param n_workers: int, optional

        :return: The fallback derivative estimate(s) as a 1D array,
            with one value per output component of the target function.
        :rtype: :class:`np.ndarray`
        """
    ...
    def recommend_fit_tolerance(self, dx=1e-3, verbose=True):
        ...
        :param dx: Offset used to probe nearby values. Default value is `1e-3`.
        :type dx: float, optional
        verbose : Whether to print out the suggested value.
            Default value is `True`.
        :type verbose: bool, optional

        :return: Recommended fit_tolerance.
        :rtype: float
        """
        try:
    ...
    def _compute_weights(self, x_vals):
        ...
        **Why not a fixed epsilon?**
        ...
        :param x_vals: Sample locations used in fitting. Shape ``(n_points,)``.
        :type x_vals: :class:`np.ndarray`

        :return: Weights of shape ``(n_points,)``. These are **not normalized**;
            downstream code (e.g., ``np.polyfit(..., w=weights)``) uses them as
            relative weights. If a normalized weight vector is required, divide
            by ``weights.sum()``.
        :rtype: :class:`np.ndarray`
        """
    ...
    def _prune_by_residuals(
        ...
        *,
        ...
        """
        Remove points whose relative residual exceeds ``fit_tolerance``,
        then refit.
        ...
        :param x_vals: Current sample abscissae.
        :type x_vals: :class:`np.ndarray`
        :param y_vals: Current sample ordinates (must be the same length as ``x_vals``).
        :type y_vals: :class:`np.ndarray`
        :param residuals: Relative residuals for the *current* fit at ``x_vals``.
        :type residuals: :class:`np.ndarray`
        fit_tolerance : float
        ...
        keep_center : bool, optional
            If True, never remove the point closest to ``central_value``. Default is `True`.
        ...
        :rtype: (:class:`np.ndarray`, :class:`np.ndarray`, bool)
        """
    ...
    def _store_diagnostics_entry(
        ...
        :param x_all: All x values used in the fit.
        :type x_all: :class:`np.ndarray`
        :param x_used: x values actually used in the fit (None if no points were used).
        :type x_used: :class:`np.ndarray`, optional
        :param y_used: y values corresponding to `x_used` (None if no points were used).
        :type y_used: :class:`np.ndarray`, optional
        :param y_fit: Fitted y values corresponding to `x_used` (None if no points were used).
        :type y_fit: :class:`np.ndarray`, optional
        :param residuals: Residuals of the fit corresponding to `y_used` (None if no points were used).
        :type residuals: :class:`np.ndarray`, optional
